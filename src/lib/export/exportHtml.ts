...
export function exportHtml(
  ...
  if (isCustomChart) {
    exportCustomSvgHtml(settings, data, columnMapping, filename, options);
  } else {
    exportApexHtml(settings, data, columnMapping, filename, options);
  }
}

function exportApexHtml(
  settings: ChartSettings,
  data: DataRow[],
  columnMapping: ColumnMapping,
  filename: string,
...
<body>
  <div id="chart-wrapper">
    ...
    <div class="footer">
      <p>${escHtml(settings.footer.sourceLabel)}: ${escHtml(settings.footer.sourceName)}</p>
    </div>` : ''}
  </div>
  <script>
    var options = ${JSON.stringify(chartOptions, null, 2)};
